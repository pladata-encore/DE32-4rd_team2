...
### 🔍최종 검토 및 개선
- **모바일 UI 개선** --> 현재 모바일 UI가 다소 미흡하므로, 향후 개발 시 모바일 사용자 경험을 고려하여 UI를 개선 필요
- **대화 기록 관리** --> 서버에 재접속할 경우 이전 대화 기록이 노출되지 않으므로, 이를 해결하여 사용자들이 연속적으로 대화를 이어갈 수 있도록 개선
- **MyBatis에서 JPA로 전환** --> MyBatis를 JPA로 변경하여 코드의 가독성과 유지보수성을 향상
- **UI 디자인 개선** --> 사용자 인터페이스(UI)가 좀 더 깔끔하고 사용하기 편리하게 디자인 개선
